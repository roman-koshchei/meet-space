...
import type {SignalRContextType} from "~/context/ConnectionContext";
...
export default function ChatArea({
    ...
    return (
        <div
            className={`w-full md:w-80 flex-1 md:flex-none overflow-y-auto lg:w-96 bg-white shadow-md p-4 flex ${activeTab === 'chat' ? 'block' : 'hidden'}`}>
            {/* Chat tabs */}
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center p-2 rounded-xl hover:bg-gray-100">
                            <Avatar className="h-10 w-10 mr-2">
                                <AvatarFallback className={`text-lg ${getColorByInitial(username)}`}>
                        ...
                        {participants.map((participantId) => (
                            <div key={participantId} className="flex items-center p-2 rounded hover:bg-gray-100">
                                <Avatar className="h-10 w-10 mr-2">
                                    <AvatarFallback className={`text-lg ${getColorByInitial(participantId)}`}>
